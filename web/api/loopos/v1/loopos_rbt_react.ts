...
// Helper for getting the expected shape of a request, i.e., either a
// Zod shape or a protobuf instance, from a protobuf shape.
const OpsTicketActivityFeedRequestFromProtobufShape = (
  partialRequest: protobuf_es.PartialMessage<Empty>
): OpsTicket.ActivityFeedRequest => {
  // TOOD: update `protoToZod()` to actually work from
  // any objects that match the shape, not just protobuf instances,
  // and then we won't need to first call `fromJson()` here.
  const request = partialRequest instanceof Empty
  ...
  return reboot_api.validate(
    "OpsTicket.methods.activityFeed.request",
    api.OpsTicket.methods.activityFeed.request,
    reboot_api.protoToZod(
      api.OpsTicket.methods.activityFeed.request,
      request
    )
...
// Helper for getting the expected shape of a request, i.e., either a
// Zod shape or a protobuf instance, from a JSON string.
const OpsTicketActivityFeedRequestFromJsonString = (
  jsonRequest: string
): OpsTicket.ActivityFeedRequest => {
  return OpsTicketActivityFeedRequestFromProtobufShape(
    Empty.fromJsonString(jsonRequest)
  );
};
...
// Helper for getting a protobuf instance for a request from the
// expected shape, i.e., either a Zod shape or a protobuf shape.
const OpsTicketActivityFeedRequestToProtobuf = (
  partialRequest?: OpsTicket.PartialActivityFeedRequest
): Empty => {
  return partialRequest instanceof Empty
    ? partialRequest
    : new Empty().fromJson(
...
const OpsTicketActivityFeedResponseFromProtobufShape = (
  partialResponse: protobuf_es.PartialMessage<loopos_pb.OpsTicketActivityFeedResponse>
) => {
  // TOOD: update `protoToZod()` to actually work from
  // any objects that match the shape, not just protobuf instances,
  // and then we won't need to first call `fromJson()` here.
  const response = partialResponse instanceof loopos_pb.OpsTicketActivityFeedResponse
    ? partialResponse
    : loopos_pb.OpsTicketActivityFeedResponse.fromJson(partialResponse);

  return reboot_api.validate(
    "OpsTicket.methods.activityFeed.response",
    api.OpsTicket.methods.activityFeed.response,
    reboot_api.protoToZod(
      api.OpsTicket.methods.activityFeed.response,
      response
    )
...
const OpsTicketActivityFeedResponseToProtobuf = (
  partialResponse?: OpsTicket.PartialActivityFeedResponse
): loopos_pb.OpsTicketActivityFeedResponse => {
  return partialResponse instanceof loopos_pb.OpsTicketActivityFeedResponse
    ? partialResponse
    : new loopos_pb.OpsTicketActivityFeedResponse().fromJson(
      reboot_api.zodToProtoJson(
        api.OpsTicket.methods.activityFeed.response,
        reboot_api.validate(
          "OpsTicket.methods.activityFeed.response",
...
export namespace OpsTicket {
  ...
  z.infer<
    reboot_api.EnsureZodObject<
      typeof api.OpsTicket.methods.activityFeed.request
      >
    >;

  export type PartialActivityFeedRequest =
  ActivityFeedRequest;

  export type ActivityFeedResponse =
  z.infer<
    reboot_api.EnsureZodObject<
      typeof api.OpsTicket.methods.activityFeed.response
      >
    >;

  export type PartialActivityFeedResponse =
...
const OPS_TICKET_ACTIVITY_FEED_ERROR_TYPES = [
  ...ERROR_TYPES,

  // Method errors.
] as const; // Need `as const` to ensure TypeScript infers this as a tuple!

export type OpsTicketActivityFeedAbortedError =
  reboot_api.InstanceTypeForErrorTypes<
    typeof OPS_TICKET_ACTIVITY_FEED_ERROR_TYPES
  >[number];

export class OpsTicketActivityFeedAborted extends reboot_api.Aborted {
  static fromStatus(status: reboot_api.Status) {
    let error = reboot_api.errorFromGoogleRpcStatusDetails(
      status,
      OPS_TICKET_ACTIVITY_FEED_ERROR_TYPES,
    );

    if (error !== undefined) {
      return new OpsTicketActivityFeedAborted(
        error, { message: status.message }
      );
    }
  ...
  constructor(
    error: OpsTicketActivityFeedAbortedError | z.infer<typeof reboot_api.ZOD_ERRORS>,
    { message }: { message?: string } = {}
  ) {
    super();
  ...
  readonly #error: OpsTicketActivityFeedAbortedError;
  readonly code: reboot_api.StatusCode;
  readonly #message?: string;
}
...
export interface UseOpsTicketApi {
  ...
  useActivityFeed(
    ...
    response: OpsTicket.ActivityFeedResponse | undefined;
    isLoading: boolean;
    aborted: OpsTicketActivityFeedAborted | undefined;
  };
  useActivityFeed(
    partialRequest?: OpsTicket.PartialActivityFeedRequest,
    options?: { suspense: true }
  ): {
    response: OpsTicket.ActivityFeedResponse;
    isLoading: boolean;
    aborted: undefined;
  } | {
    response: undefined;
    isLoading: boolean;
    aborted: OpsTicketActivityFeedAborted;
  };
  useActivityFeed(
    partialRequest?: OpsTicket.PartialActivityFeedRequest,
    options?: { suspense: false }
...
class OpsTicketInstance {
  ...
  private useActivityFeedReaders: {
    [requestBearerTokenHash: string]: reboot_react.Reader<loopos_pb.OpsTicketActivityFeedResponse>
  } = {};

  private activityFeedFinalizationRegistry =
    new FinalizationRegistry((finalize) => finalize());
  ...
  startActivityFeed(
    ...
    let reader = this.useActivityFeedReaders[requestBearerTokenHash];
    ...
    if (reader === undefined) {
      ...
      reader = {
        ...
        setResponse(
          response: loopos_pb.OpsTicketActivityFeedResponse,
          { cache }: { cache: boolean } = { cache: true }
        ) {
          // Store the response, delete the status.
          this.response = response;
          ...
          if (cache && offlineCacheEnabled) {
            ...
            reboot_web.offlineCache().set(cacheKey, cachedResponse)
              .catch((error) => {
                console.warn(
                  `[Reboot] Setting of offline reader cache entry for 'OpsTicket.ActivityFeed' errored with ${error}`
                );
              });
          }
        },
      ...
      this.activityFeedFinalizationRegistry.register(
        promise,
        () => {
          if (!reader.used) {
            delete this.useActivityFeedReaders[requestBearerTokenHash];
            reader.abortController.abort();
          }
        }
      );
      ...
      // We want to remove the promise so that it can be garbage collected
      // which is our indication that React is no longer using it. But this
      // races with calls to `useActivityFeed(...)`
      // that might be adding their `setResponse`, `setIsLoading`, etc, so
      // we delay deleting the promise for at least a second.
      //
      // Note that deleting the promise is okay because all subsequent calls
      ...
      // undefined.
      reader.promise.then(async () => {
        // Allow the call to `useActivityFeed(...)`
        // at least 5 seconds to indicate that the reader is being used,
        // afterwhich, once `reader.promise` gets garbage collected
        // we'll know that it must have been abandoned by React, e.g.,
        // because the component was suspended and never committed.
      ...
      this.useActivityFeedReaders[requestBearerTokenHash] = reader;
      ...
      if (offlineCacheEnabled) {
        ...
        reboot_web.offlineCache().get(cacheKey).then((cachedResponse) => {
          if (cachedResponse !== null) {
            ...
            if (reader.response === undefined) {
              reader.setResponse(
                loopos_pb.OpsTicketActivityFeedResponse.fromJsonString(cachedResponse),
                { cache: false } // Don't re-cache the value!
              );
            }
          }
          ...
          console.warn(
            `[Reboot] Retrieval of offline reader cache entry for 'OpsTicket.ActivityFeed' errored with ${error}`
          );
        });
      }
    }
  ...
  useActivityFeed(
    ...
    offlineCacheEnabled: boolean,
    cacheKey: string | null,
    setResponse: (response: loopos_pb.OpsTicketActivityFeedResponse) => void,
    setIsLoading: (isLoading: boolean) => void,
    setStatus: (status: reboot_api.Status) => void
  ) {
    // We need to call start here because with strict mode the
    ...
    // which will mean the next time the `useEffect` calls here
    // we'll create a new reader in start.
    const reader = this.startActivityFeed(
      requestBearerTokenHash,
      serializedRequest,
      bearerToken,
      offlineCacheEnabled,
  ...
  unuseActivityFeed(
    ...
    const reader = this.useActivityFeedReaders[requestBearerTokenHash];
...
export const useOpsTicket = (
  ...
  function useActivityFeed(
    ...
    const newRequest = OpsTicketActivityFeedRequestToProtobuf(partialRequest);
    ...
    const cacheKey: string | null = useMemo(
      () => {
        if (offlineCacheEnabled) {
          return `${stateRef}:ActivityFeed:${requestHash}`;
        }
        return null;
      },
      [stateRef, offlineCacheEnabled, requestHash]
    ...
    // render where one of them will be defined, i.e., after we've
    // waited for `reader.promise` via `React.use()`.
    const reader = instance.startActivityFeed(
      requestBearerTokenHash,
      serializedRequest,
      bearerToken,
      offlineCacheEnabled,
    ...
    const [response, setResponse] = useState<
      OpsTicket.ActivityFeedResponse | undefined
      >(reader.response && OpsTicketActivityFeedResponseFromProtobufShape(reader.response));

    const [aborted, setAborted] = useState<
      OpsTicketActivityFeedAborted | undefined
      >(reader.status && OpsTicketActivityFeedAborted.fromStatus(reader.status));
    ...
    useEffect(() => {
      ...
      instance.useActivityFeed(
        ...
        (response: loopos_pb.OpsTicketActivityFeedResponse) => {
          setAborted(undefined);
          setResponse(OpsTicketActivityFeedResponseFromProtobufShape(response));
        },
        setIsLoading,
        (status: reboot_api.Status) => {
          // If the server rejected us due to an expired
          ...
          const aborted = OpsTicketActivityFeedAborted.fromStatus(status);

          console.warn(
            `[Reboot] 'OpsTicket.ActivityFeed' aborted with ${aborted.message}`
          );
      ...
      return () => {
        instance.unuseActivityFeed(id, requestBearerTokenHash);
      };
    }, [
      instance,
      serializedRequest,
  ...
  async function activityFeed(
    ...
    const request = OpsTicketActivityFeedRequestToProtobuf(partialRequest);
      ...
      while (true) {
        try {
          ...
          return {
            response: await reboot_web.guardedFetch(
              new Request(
                `${rebootClient.url}/__/reboot/rpc/${stateRef}/loopos.v1.OpsTicketMethods/ActivityFeed`, {
                  method: "POST",
                  headers,
                  body: request.toJsonString()
                }
          ...
          if (options?.signal?.aborted || !retry) {
            const aborted = new OpsTicketActivityFeedAborted(
              new reboot_api.errors_pb.Aborted(), {
                message: e instanceof Error
                  ? `${e}`
                  : `Unknown error: ${JSON.stringify(e)}`
        ...
        await backoff.wait(`[Reboot] Retrying call to \`loopos.v1.OpsTicketMethods.ActivityFeed\` with backoff...`);
      }
    })();
    ...
    if (aborted) {
      ...
      if (newToken) {
        ...
        try {
          ...
          if (retryResponse.ok) {
            return {
              response:
                OpsTicketActivityFeedResponseFromProtobufShape((loopos_pb.OpsTicketActivityFeedResponse.fromJson(await retryResponse.json())))
            };
          }
          // Fall through to generic error handling on retry failure.
          return {
            aborted: new OpsTicketActivityFeedAborted(
              new reboot_api.errors_pb.Unknown(), {
                message: `Unknown error with HTTP status ${retryResponse.status} after token refresh`
              }
            )
      ...
      return {
        response:
          OpsTicketActivityFeedResponseFromProtobufShape((loopos_pb.OpsTicketActivityFeedResponse.fromJson(await response.json())))
      };
    }
  }
