...
from rbt.v1alpha1.tasks_pb2 import (
    CancelTaskRequest,
    CancelTaskResponse,
    ListTasksRequest,
    ListTasksResponse,
...
def MakeLegacyGrpcServiceable(
    ...
    if issubclass(servicer_type, rbt.v1alpha1.tasks_pb2_grpc.TasksServicer):
        return TasksServiceable(servicer_type)
    raise ValueError(f"Unknown legacy gRPC servicer type '{servicer_type}'")


class TasksServiceable(IMPORT_reboot_aio_servicers.Serviceable):
    """ A Servicable that makes it possible to host a legacy gRPC
    `TasksServicer` on Reboot."""

    def __init__(self, legacy_grpc_servicer_type: type[rbt.v1alpha1.tasks_pb2_grpc.TasksServicer]):
        self._legacy_grpc_servicer_type = legacy_grpc_servicer_type

    def service_names(self) -> list[IMPORT_reboot_aio_types.ServiceName]:
        return [IMPORT_reboot_aio_types.ServiceName('rbt.v1alpha1.Tasks')]
    ...
    def file_descriptor(self) -> IMPORT_google_protobuf_descriptor.FileDescriptor:
        return rbt.v1alpha1.tasks_pb2.DESCRIPTOR

    def instantiate(self) -> tuple[
        rbt.v1alpha1.tasks_pb2_grpc.TasksServicer,
        ...
        add_to_server_method = rbt.v1alpha1.tasks_pb2_grpc.add_TasksServicer_to_server
